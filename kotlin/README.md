...
- `src/main/kotlin/ttscache/TtsCache.kt` — `TtsCache` object plus
  `TtsCacheConfig` and `SynthesizeRequest`. No external dependencies,
  JDK only (`java.security`, `java.util`).
...
No networking yet. The contract part (normalize, key, split,
request builder) is done; the thin `HttpURLConnection` fetch wrapper
is intentionally left out until the contract is green on a real
toolchain. `synthesizeRequest` already produces the URL and JSON body
a future fetcher will send.
